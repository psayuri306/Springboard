...
select Facilities.name as facility, concat(firstname, ' ', surname) as name, 
...
SELECT m.surname, m.firstname, r.surname AS recomender_surname, r.firstname AS recomender_firstname
FROM Members AS m
...
select f.name as 'Court', count(b.memid) as 'Member Count'
from (select facid, memid
      from Bookings where memid != 0) as b
left join Facilities as f on b.facid = f.facid
group by b.facid
order by count(b.memid) ;
